...
export async function buildChatBootstrap(): Promise<ChatBootstrapResponse> {
  ...
  const resolvedDefaultAgentId =
    resolvedList.find((row) => Boolean(row.isDefault ?? row.default))?.id ||
    parsedList.find((row) => Boolean(row.isDefault ?? row.default))?.id ||
  ...
  const agents = [...agentIds]
    .map((id) => {
      ...
      return {
        ...
        model,
        isDefault: Boolean(merged.isDefault ?? merged.default) || id === resolvedDefaultAgentId,
        workspace:
