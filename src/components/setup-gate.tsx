...
export function SetupGate({ children }: { children: React.ReactNode }) {
  const [status, setStatus] = useState<{ configured: boolean; hasModel: boolean; hasChannel: boolean; hasApiKey: boolean } | null>(null);
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      ...
      setStatus({ configured: data.configured, hasModel: data.hasModel, hasChannel: data.hasChannel, hasApiKey: data.hasApiKey });
    } catch {
      setError(true);
    } finally {
      setLoading(false);
